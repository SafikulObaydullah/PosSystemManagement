var MassStockGrid = {
    ...
    TotalCount: -1,
    Reset: function () {
        MassStockGrid.CurrentPage = 1;
        MassStockGrid.TotalCount = -1;
        MassStockGrid.LoadMassStockGrid();
    },
    GetStock: function (productid,id) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#new_stock_qty' + id).val(data.quantity)
            }
        })
    },
    LoadMassStockGrid: function () {
        if (MassStockGrid.LoaderAjax && MassStockGrid.LoaderAjax.readyState != 4) {
            return;
        }
        if (MassStockGrid.TotalCount == $("tr.data").length) {
            return;
        }
        ...
        var paramlite = {
            ...
            EndDate: ToDateTime,
            PageNo: MassStockGrid.CurrentPage,
            Searchtext: SearchTxt,
            CategoryName: category,
        };
        MassStockGrid.LoaderAjax = $.ajax({
            type: "post",
            url: '/Product/ProductMassStock',
            data: paramlite,
            cache: false,
            success: function (data) {
                ...
                MassStockGrid.TotalCount = TotalCount[0].TotalCount;
                var empTemplate = $("#massstocktemplate").html();
                var sourceHtml = Handlebars.compile(empTemplate);
                $("#total_count").text(TotalCount[0].TotalCount)
                if (MassStockGrid.CurrentPage == 1) {
                    $(".load_mass_stock_list_data").html(sourceHtml(dataa));
                } else {
                    $(".load_mass_stock_list_data").append(sourceHtml(dataa));
                }
                $(".load_mass_stock_list_data").show();
                $("#revdatefilter").addClass('display_nn');
                MassStockGrid.CurrentPage++;
            },
    ...
    LoadProductFromMassStock: function (id) {
        OpenTopToBottomModal("/Product/add?id=" + id);
    },
    LoadCategoryFromMassStock: function (id) {
        OpenRightToLeftModal("/Productcategorie/add?id=" + id);
    },
    //UpdateMassStockFromEnterKey: function (e, id, item) {
    //            var newqty = parseInt($('#new_stock_qty' + id).val());
    ...
    DownloadInventoryList: function () {
        var FromDateTime = $("#ReviewFilterStartDate").val();
        var ToDateTime = $("#ReviewFilterEndDate").val();
        var SearchTxt = $("#SearchOrder").val();
        var category = $("#Category").val();
        var parm = '?StartDate=' + FromDateTime
            ...
            + '&Searchtext=' + SearchTxt
            + '&CategoryName=' + category;
        window.location.href = "/inventory/inventory-download-excel" + parm;
    },
    SearchMassStockGrid: function () {
        MassStockGrid.CurrentPage = 1;
        MassStockGrid.TotalCount = -1;
        MassStockGrid.LoadMassStockGrid();
    },
                ...
                success: function (data) {
                    if (data.success) {
                        for (var i = 0; i < Approvedlist.length; i++) {
                            ...
                            if (isNaN(qty)) { qty = 0; }
                            $('#qty' + Approvedlist[i].Id).text(Approvedlist[i].Quantity);
                            $('#new_stock_qty' + Approvedlist[i].Id).val('')
                            $('.Udropdownlist' + Approvedlist[i].Id).val('')
                        }
...
$(document).ready(function () {
    ...
    $("#SearchOrder").keyup(function () {
        MassStockGrid.SearchMassStockGrid();
    })
...
$(".low_stock_inner").scroll(function () {
    if (window.innerHeight - 205 + $(".low_stock_inner").scrollTop() > $("tr.data:last").position().top) {
        MassStockGrid.LoadMassStockGrid();
    }
});