var PropertyProductSuggestiontemplate =
    ...
    + '<span <img class="sugg_img" src="{6}{4}"/></span><br>'
...
var SearchedProducttemplate =
    ...
    '<span class="card-title">{0},<img class="sugg_img" src="{15}{14}"/></span><br>' +
    '<span class="card-title">{1}</span><br>' +
    '<span class="card-text" style="font-style: italic;">Qty: 1 {2} ,<span> Price: K{3}</span><span class="float-right">{5}</span></span><br>' +
...
var ChangeQty = function (id,rn) {
    ...
    $.ajax({
        url: "/Product/GetProductQtyCustomWH",
    ...
    if (e.keyCode == 13) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    console.log(data)
                    var selecttemplate = '<select class="form-control product-unit unit_pro_' + id + i + '" title="' + data.result.secondaryUnitInt + '" onchange = "ChangeQty(' + id + ',' + i + ')">';
                    var optiontemplate = '<option value="' + data.result.secondaryUnit + '.S">' + data.result.secondaryUnit + ' [S]</option>';
                    ...
                    setTimeout(function () {
                        var NewPosAddFromSuggRowCustom = "<tr data-id='{3}' class='newp_{3}'>"
                            ...
                            + "<td valign='top'>"
                            + "<div class='input-group'>"
                            + selecttemplate + "<input value='1' oninput='validity.valid || (value = 1)' type='number' min='0' class='form-control product-qty pq_{3}_{17}' id='Qty' placeholder='0' onKeyup='ChangeQty({3},{17})'  onchange='ChangeQty({3},{17})' onclick='ChangeQty({3},{17})'/>"
                            + "</div>"
                            ...
                            + "</td>"
                            + "<td valign='top'><div class='input-group'><div class='input-group-prepend'><div class='input-group-text'><span class='currency'>K</span></div></div><input value='{12}' type='text' class='form-control product-price pp_{3}_{17}' readonly='readonly' datarequired = 'true' onKeyup='OrderPosSalesRowKeyUpPrice(this,event)' placeholder='Rate' /> <input value='{12}' type='hidden' class='fixed-price' /></div></td>"
                            ...
                            + "<td valign='top'><div class='input-group'><div class='input-group-prepend'><div class='input-group-text'><span class='currency'>K</span></div></div><input value='{8}' readonly='readonly' placeholder='0' oninput='validity.valid || (value = 1)' type='number'  min='0' class='form-control product-discount pd_{3}_{17}' /*onKeyup='OrderPosSalesRowKeyUpQty(this,event)' onchange='OrderPosSalesRowKeyUpQty(this,event)'*//><input value='{8}' type='hidden' class='fixed-product-discount' /><input value='{8}' type='hidden' class='whole-product-discount' /></div></td>"
                        ...
                        var tAmt = 0, rate = parseFloat(data.result.wholeSaleSecondaryPrice), vat = parseFloat(data.result.vatAmount), dis = parseFloat(data.result.wholesaleDiscountAmount);
                        ...
                        /*6*/ tAmt.toFixed(2),
                        /*7*/ vatper,
                        /*8*/ data.result.wholesaleDiscountAmount,
                        /*9*/ data.result.tertiaryUnitInt,
                        /*10*/ data.result.wholeSaleTertiaryPrice,
                        /*11*/ data.result.secondaryUnitInt,
                        /*12*/ data.result.wholeSaleSecondaryPrice,
                ...
                if (data.success) {
                    ...
                    setTimeout(function () {
                        var NewPosAddFromSuggRowCustom = "<tr data-id='{3}' class='newp_{3}'>"
                            ...
                            + "<input type='hidden' class='terwholesale-tdis' value='{8}'>"
                            + "<input type='hidden' class='wholesale-qty' value='{9}'>"
                            + "<input type='hidden' class='wholesaleter-value' value='{10}'>"
                            + "<input type='hidden' class='secondary-qty' value='{11}'>"
                            + "<input type='hidden' class='whsecondary-value' value='{12}'>"
                            + "<input type='hidden' class='primary-name' value='{1}'>"
                            + "<input type='hidden' class='secondary-name' value='{13}'>"
                            + "<input type='hidden' class='wholesale-name' value='{14}'>"
                            + "<input type='hidden' class='discounttype' value='{15}'>"
                            ...
                            + "</td>"
                            + "<td valign='top'><div class='input-group'><div class='input-group-prepend'><div class='input-group-text'><span class='currency'>K</span></div></div><input value='{12}' type='text' class='form-control product-price pp_{3}_{17}' readonly='readonly' datarequired = 'true' onKeyup='OrderPosSalesRowKeyUpPrice(this,event)' placeholder='Rate' /> <input value='{12}' type='hidden' class='fixed-price' /></div></td>"
                            + "<td valign='top'><div class='input-group'><div class='input-group-prepend'><div class='input-group-text'><span class='currency'>K</span></div></div><input value='{18}' type='text' class='form-control vat-price pv_{3}_{17}' readonly='readonly' placeholder='0' onKeyup='OrderPosSalesRowKeyUpPrice(this,event)' placeholder='Vat' /><input value='{18}' type='hidden' class='fixed-vat-price' /><input value='{18}' type='hidden' class='whole-vat-price' /></div></td>"
                        ...
                        var searchresultstring = "";
                        //var vat2amt = 0;
                        //var vat2 = parseFloat(data.result.wholeSaleSecondaryPrice) / parseInt(data.result.secondaryUnitInt);
                        //if (data.result.vatType = "percent") {
                        //    vat2amt = ((vat2 * data.result.vatPercent) / 100) * data.result.secondaryUnitInt;
                        //}
                        //else
                        //{
                        //    vat2amt = data.result.vatAmount * data.result.secondaryUnitInt;
                        //}
                        var tAmt = 0, rate = parseFloat(data.result.wholeSaleSecondaryPrice), vat = parseFloat(data.result.vatAmount),  dis = parseFloat(data.result.wholesaleDiscountAmount);
                        if (isNaN(rate)) { rate = 0; }
                        if (isNaN(vat)) { vat = 0; }
            ...
            if (data.result != "") {
                ...
                if (resultparse.length > 0) {
                    ...
                    for (var i = 0; i < resultparse.length; i++) {
                        ...
                        /*3*/ resultparse[i].SalesPrice,
                        /*4*/ resultparse[i].Id,
                       /*5*/(resultparse[i].ProductType == "Service" ? "" : resultparse[i].Alias2),
                        /*6*/ resultparse[i].WholeSaleSecondaryPrice,
                        /*7*/ resultparse[i].WholeSaleTertiaryPrice,
        ...
        if (CommonUiValidation() && PartialpaymentValidation() && DiscountValidation() && CardandmobilepaymentValidation()) {
            ...
            if (ListProductsCustomPOS.length > 0) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            invoice = $("#Invoice_InvoiceId").val()
                            if (ispos) {
                                OpenTopToBottomConfirmationModal('/sales/pos-print?invoiceid=' + invoice)
                            } else {
                                OpenTopToBottomModal('/Sales/SalesPrintPreviewPage?ids=' + invoice)
                            }
                            $('#NewPOSSales').click()
