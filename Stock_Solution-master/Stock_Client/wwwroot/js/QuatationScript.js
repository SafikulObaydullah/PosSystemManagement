            ...
            if (data.result != "") {
                ...
                if (resultparse.length > 0) {
                    ...
                    for (var i = 0; i < resultparse.length; i++) {
                        ...
                        /*3*/ resultparse[i].SalesPrice,
                        /*4*/ resultparse[i].Id,
                       /*5*/(resultparse[i].ProductType == "Service" ? "" : resultparse[i].ProductType != null ? resultparse[i].Alias2 : ""),
...
var POSSale = {
    SaveNewPOSSale: function () {
        ...
        if (CommonUiValidation() && PartialpaymentValidation() && DiscountValidation() && CardandmobilepaymentValidation()) {
            ...
            var requestData = {
                Id: $("#QuatationInvoice_Id").val(),
                CustomerIntId: $('#customer_id').val(),
                CustomerName: $('#customer_name').text(),
                SalesCounter: $('#SalesCounter').val(),
                InvoiceId: $("#QuatationInvoice_InvoiceId").val(),
                VoucherNumber: $("#QuatationInvoice_VoucherNumber").val(),
                ...
                QuatationInvoiceDetailList: ListProductsCustomPOS
            };
            $.ajax({
                url: "/Inventory/SaveNewQuatation",
                type: 'post',
                data: requestData,
                beforeSend: function () {
        ...
        if (CommonUiValidation() && PartialpaymentValidation() && DiscountValidation() && CardandmobilepaymentValidation()) {
            ...
            if (ListProductsCustomPOS.length > 0) {
                var requestData = {
                    ...
                    SalesCounter: $('#SalesCounter').val(),
                    InvoiceId: $("#QuatationInvoice_InvoiceId").val(),
                    SalesDate: $("#QuatationInvoice_SalesDate").val(),
                    VoucherNumber: $("#QuatationInvoice_VoucherNumber").val(),
                    TransactionType: $("#TransactionType").val(),
                    TransactionId: $("#TransactionId").val(),
                    PartialAmount: $("#PartialAmount").val(),
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            if (ispos) {
                                OpenTopToBottomConfirmationModal('/sales/pos-print?InvoiceId=' + $("#QuatationInvoice_InvoiceId").val())
                            } else {
                                OpenTopToBottomModal('/Sales/SalesPrintPreviewPageforqoutation?ids=' + $("#QuatationInvoice_InvoiceId").val(), function () {
